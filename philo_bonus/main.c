...
/*   Updated: 2021/06/12 14:22:03 by mamoussa         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	main(int argc, char **argv)
{
	...
	philos = (t_philo*)malloc(sizeof(t_philo)); /* struct to store informations about each philo */
	sem_unlink("/philo_bonus_forks");
	sem_unlink("/philo_bonus_print");
	philos->sem = sem_open("/philo_bonus_forks",O_CREAT | O_EXCL, S_IRUSR | S_IWUSR,
	data->number_of_philo);
	philos->print = sem_open("/philo_bonus_print",O_CREAT | O_EXCL, S_IRUSR | S_IWUSR, 1);
	if (main_helper(philos, data))
		return (1);
	/*******************************************************************/
	free_philo(philos);
	free_data(data);
